...
logo: `<img src="./images/AvengedSevenfold.jpg" class="w-full h-full object-contain" />`
    ...
    function createDayElement(day, isOtherMonth, events, isToday = false) {
      ...
      if (events.length > 0 && !isOtherMonth) {
        events.forEach(event => {
          ...
          logoContainer.className = 'w-14 h-14 bg-white rounded-xl shadow-lg flex items-center justify-center event-indicator overflow-hidden';
          logoContainer.innerHTML = event.logo;
          
          const eventTitle = document.createElement('p');
          eventTitle.className = 'text-xs text-purple-300 font-bold text-center truncate w-full';
    ...
    function openEventModal(event) {
      ...
      if (event.originalPrice > event.price) {
        const discount = Math.round(((event.originalPrice - event.price) / event.originalPrice) * 100);
    ...
    function openReservationModal() {
      ...
      const reservationEventTitle = document.getElementById('reservation-event-title');
    ...
    function generatePassengerForms(numPassengers) {
      ...
      for (let i = 1; i <= numPassengers; i++) {
        ...
        passengerForm.className = 'bg-slate-800/50 border border-purple-500/20 rounded-2xl p-5 mb-4';
    ...
    function sendToWhatsApp() {
      ...
      if (!passengers) {
        ...
        sendButton.innerHTML = '<span class="text-red-200">⚠️ Por favor completa todos los campos</span>';
    ...
    document.getElementById('show-reservation-form').addEventListener('click', () => {
      closeEventModal();
      openReservationModal();
    });

    document.getElementById('close-reservation').addEventListener('click', closeReservationModal);
