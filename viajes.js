...
const trips = [
  ...
  {
    id: 2,
    artist: "IL VOLO LIVE IN CONCERT",
    tour: "Tour 2026",
    city: "Auditorio Nacional, CDMX",
      dates: ["3 de Marzo, 2026"],
    schedules: ["2:00 PM"],
  ...
  },
  {
    ...
    city: "Teatro Metropolitan, CDMX",
      dates: ["05 de Marzo, 2026"],
    schedules: ["11:30 AM"],
    image: "./images/lykn.jpg",
    price: 629,
    originalPrice: 655,
    includes: [
      "Viaje redondo",
  ...
  },
  {
    ...
    city: "Pepsi Center, CDMX",
     dates: ["07 de Febrero, 2026"],
    schedules: ["2:00 PM"],
    image: "./images/tvgirl.jpg",
    price: 629,
    originalPrice: 655,
    includes: [
      "Viaje redondo",
  ...
  {
    id: 5,
    artist: "Shine Mexico City Fancon",
    tour: "Tour 2026",
    city: "Teatro Metropolitan, CDMX",
     dates: ["10 de Marzo, 2026"],
    schedules: ["11:30 AM"],
  ...
  },
  {
    ...
    city: "Pepsi Center WTC, CDMX",
      dates: ["11 de Marzo, 2026", "12 de Marzo, 2026"],
    schedules: ["2:00 PM"],
...
${
  trip.image.includes("/")
    ? `<img 
       ...
       />`
    : `<span class="text-7xl float-animation">${trip.image}</span>`
          ...
          <div class="p-5">
            <div class="flex items-start justify-between mb-2">
              <div>
                <h3 class="text-xl font-bold">${trip.artist}</h3>
                <p class="text-purple-400 text-sm">${trip.tour}</p>
              </div>
            </div>
            ...
            <div class="flex items-end justify-between">
              <div>
                ${
                  trip.originalPrice > trip.price
                    ? `
                  <span class="text-gray-500 line-through text-sm">$${trip.originalPrice.toLocaleString()}</span>
                `
                    : ""
                }
                <p class="text-2xl font-bold text-pink-400">$${trip.price.toLocaleString()}<span class="text-sm text-gray-400 font-normal"> MXN</span></p>
              </div>
...
${
  trip.image.includes("/")
    ? `<img 
       ...
       />`
    : `<span class="text-6xl mb-4 block">${trip.image}</span>`
}
          <h3 class="text-2xl font-bold">${trip.artist}</h3>
          <p class="text-purple-400">${trip.tour}</p>
        </div>
        
        <div class="space-y-4 mb-6">
          <div class="flex justify-between items-center py-3 border-b border-white/10">
            <span class="text-gray-400">📍 Destino</span>
            <span class="font-semibold">${trip.city}</span>
          </div>
          <div class="flex justify-between items-center py-3 border-b border-white/10">
            <span class="text-gray-400">📅 Fecha</span>
            <span class="font-semibold">${trip.date}</span>
          </div>
          <div class="flex justify-between items-center py-3 border-b border-white/10">
            <span class="text-gray-400">👥 Lugares disponibles</span>
            <span class="font-semibold text-pink-400">${trip.spots}</span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between mb-6">
          <div>
            ${
              trip.originalPrice > trip.price
                ? `
              <span class="text-gray-500 line-through">$${trip.originalPrice.toLocaleString()}</span>
            `
                : ""
            }
            <p class="text-3xl font-bold text-pink-400">$${trip.price.toLocaleString()} <span class="text-sm text-gray-400 font-normal">MXN</span></p>
...
<p class="text-sm text-gray-400">
  ...
  document.getElementById("pasajeros").addEventListener("change", function (e) {
    ...
    if (count) {
      renderPassengerForms(count, trip);
    }
  });
...
function renderPassengerForms(count, trip) {
...
function sendWhatsApp(tripId) {
  ...
  const totalPrice = trip.price * pasajeros;
  const depositPrice = Math.round(totalPrice * 0.3);
